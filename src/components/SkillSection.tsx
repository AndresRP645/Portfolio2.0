...
const SkillSection: React.FC<SkillSectionProps> = ({ skillsData, theme }) => {
  const getSkillIconSrc = (theme: string, skill: Skill) => {
    if (
      theme === "dark" &&
      (skill.title.includes("Next") || skill.title.includes("Express") || skill.title.includes("GameMaker"))
    ) {
      return skill.icon[1];
    } else if (skill.title !== "Next" && skill.title !== "Express" && skill.title !== "GameMaker") {
      return skill.icon;
    } else {
      return skill.icon[0];
    }
  ...
  return (
    ...
    >
      ...
      {skillsData[0].skills.map((skill, index) => (
        <div
          key={index}
          className={`skill-item cursor-pointer flex flex-col gap-6 rounded-2xl p-5 border-solid border-[2px]  text-center max-lg:items-center skills justify-center  ${
